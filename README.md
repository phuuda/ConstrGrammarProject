...
* подтверждается взаимосвязь между одушевленностью Y (y_animacy) и тоном (pos/neg), хотя вместе у них тоже очень слабая predictive power (но чуть лучше, чем у переменной y_word)

* на основании этого, гипотеза о том, что <i>X, а не Y</i> чаще употребляется при положительной тональности, не отвергается, хотя одушевленность Y влияет на WO сильнее:

* inanimate Y -> <i>X, а не Y</i>
* animate Y + pos -> <i>X, а не Y</i>
* animate Y + neg -> <i>не Y, а X</i>

* VarImp обещал большую зависимость WO от переменной y_word, что не подтвердилось (примерно 50% prediction accuracy, i.e. absolutely unreliable)

* при этом y_word вытесняет тональность (pos, neut), т.к. random trees are biased in favor of attributes with more levels! 
